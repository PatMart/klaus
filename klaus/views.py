...
def repo_list():
    ...
    if len(repos) == 1:
        return redirect(url_for('history', repo=repos[0].name, commit_id='master', path=''))
    else:
...
class BaseRepoView(View):
    ...
    def make_context(self, repo, commit_id, path):
        ...
        readme = repo.get_tree(commit, "/README.html")
        ...
        self.context = {
            ...
            'header': readme.data if hasattr(readme, 'data') else "",
        }
...
class HistoryView(TreeViewMixin, BaseRepoView):
    ...
    def make_context(self, *args):
        ...
        if page:
            self.context['last_commit_renamed_path'] = None
            ...
            self.context['last_commit_renamed_path'] = last_commit_renamed_path
